...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CHAINCODEID -v $VERSION -c '{"Args":["init","a","100","b","200"]}' -P "$POLICY" >&log.txt
	else
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CHAINCODEID -v $VERSION -c '{"Args":["init","a","100","b","200"]}' -P "$POLICY" >&log.txt
	fi
	res=$?
	cat log.txt
...
if [ $# -lt 2 ] ; then
	...
	echo
	echo "Example: instantiate 1.1 lomcode lom_simple"
	echo 
